...
function ADVR.onMeleeWeaponCollide(flail, collision)
	...
	if collision.relativeVelocity.magnitude > 2 then
		local expl = game.SpawnObjectLocal(objects.EFFECT_EXPLOSION_SMALL, flail.transform.position).GetComponent_Explosion_()
		expl.damage = player.GetSecondaryDamageWithPickupModifiers()
		expl.enableCritChance = true
		pickup.SendRPCEvent(1, flail.transform.position, rpcType.TO_PROXIES)
	end
end
