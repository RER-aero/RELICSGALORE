function ADVR.onLoad()
    ...
    pickup.desc = "Bomb bounces cause an explosion, but melee damage is decreased"
...
function ADVR.onMeleeWeaponCollide(bomb, collision)
	...
	if collision.relativeVelocity.magnitude > 2 then
		local expl = game.SpawnObjectLocal(objects.EFFECT_EXPLOSION_SMALL, bomb.transform.position).GetComponent_Explosion_()
		expl.damage = player.GetSecondaryDamageWithPickupModifiers()
		expl.enableCritChance = true
		pickup.SendRPCEvent(1, bomb.transform.position, rpcType.TO_PROXIES)
	end
end
