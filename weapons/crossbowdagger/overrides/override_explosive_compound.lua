function ADVR.onLoad()
    ...
    pickup.desc = "Punch attacks cause an explosion, but melee damage is decreased"
...
function ADVR.onMeleeWeaponCollide(sword, collision)
	...
	if collision.relativeVelocity.magnitude > 2 then
		local expl = game.SpawnObjectLocal(objects.EFFECT_EXPLOSION_SMALL,  sword.transform.position).GetComponent_Explosion_()
		expl.damage = player.GetSecondaryDamageWithPickupModifiers()
		expl.enableCritChance = true
		pickup.SendRPCEvent(1, sword.transform.position, rpcType.TO_PROXIES)
	end
end

function ADVR.onRPCReceived(eventID, position)
	if eventID == 1 then
